...
export class DiscoveryJobService {
  ...
  constructor(private readonly flights: FlightProvider, private readonly accommodation: AccommodationProvider, private readonly destinations: DestinationService, private readonly ttlMs = 15 * 60 * 1000, private readonly routing?: RoutingProvider, private readonly cars?: CarRentalProvider) {}
  ...
  private async run(job: DiscoveryJob) { try { for (const stage of ['DATES', 'AIRPORTS', 'FLIGHTS', 'ACCOMMODATION', 'RANKING'] as DiscoveryStage[]) this.emit(job, stage); job.result = await discoverTrips(job.query, this.flights, this.accommodation, this.destinations,this.routing,this.cars); job.status = 'COMPLETE'; this.emit(job, 'COMPLETE'); } catch (error) { job.status = 'FAILED'; job.error = error instanceof Error ? error.message : 'Error inesperado'; this.emit(job, 'FAILED'); } }
}
